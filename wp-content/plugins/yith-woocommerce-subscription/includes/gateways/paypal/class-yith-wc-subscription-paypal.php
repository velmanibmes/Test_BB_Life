...
if ( ! class_exists( 'YWSBS_Subscription_Paypal' ) ) {
	...
	class YWSBS_Subscription_Paypal {
		...
		public function __construct() {
			...
			if ( ! apply_filters( 'ywsbs_load_paypal_standard_handler', isset( $settings['enabled'] ) && 'yes' === $settings['enabled'] ) ) {
				return;
			}
		...
		public function subscription_args( $args ) {
			...
			if ( count( $item_names ) > 1 ) {
				// translators: placeholder order number.
		...
		public function cancel_recurring_payment( $result, $subscription ) {
			...
			if ( $this->debug ) {
				$this->wclog->add( 'paypal', 'YSBS - Subscription Cancel Request: #' . $subscription->id . '. Details of response: ' . print_r( $response, true ) ); //phpcs:ignore
			}
